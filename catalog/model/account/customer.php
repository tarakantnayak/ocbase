...
//		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW(), user_name_type = '" . $data['user_name_type'] . "'");
		
		if ($data['user_name_type'] == 'M'){
		    $mobile_no = $data['user_name'];
		} else {
		    $mobile_no = '';
		}
		
		if ($data['user_name_type'] == 'E'){
		    $email_id = $data['user_name'];
		} else {
		    $email_id = '';
		}
		
		
		//$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', email = '" . $this->db->escape($data['username']) . "', telephone = '" . $this->db->escape($mobile_no) . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW(), user_name_type = '" . $data['user_name_type'] . "'");
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', email = '" . $email_id . "', telephone = '" . $this->db->escape($mobile_no) . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW(), user_name_type = '" . $data['user_name_type'] . "', user_name = '" . $data['user_name'] . "'");
	...
	public function editPassword($user_name, $password) {
	    $this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($password)))) . "', code = '' WHERE LOWER(user_name) = '" . $this->db->escape(utf8_strtolower($user_name)) . "'");
	}
	...
	public function getLoginAttempts($user_name) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_login` WHERE email = '" . $this->db->escape(utf8_strtolower($user_name)) . "'");
	...
	public function getTotalCustomersByUsername($user_name , $user_name_type) {
	    $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(user_name) = '" . $this->db->escape(utf8_strtolower($user_name)) . "'");

	    if ($query->row['total'] == 0){
	        if ($user_name_type == 'E'){
	            $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($user_name)) . "'");
	        } else {
	            $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE telephone = '" . (int)$user_name . "'");
	        }
	    }
	    return $query->row['total'];
	}
	public function getCustomerByUserName($user_name) {
	    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(user_name) = '" . $this->db->escape(utf8_strtolower($user_name)) . "'");
