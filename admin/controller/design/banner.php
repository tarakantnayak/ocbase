...
class ControllerDesignBanner extends Controller {
    ...
    protected function getForm() {
      ...
      //  $logger->write("prining banner images");
      //  $logger->write($banner_images);
        ...
        foreach ($banner_images as $key => $value) {
            foreach ($value as $banner_image) {
                ...
                $banner_image_content = $this->model_design_banner->getImageContent($banner_image['banner_image_id']);
                
                
                
                $image_content_data = array();
                
                foreach ($banner_image_content as $content_data) {
                ...
                $data['banner_images'][$key][] = array(
                    ...
                    'width' => $banner_image['image_width'],
                    'height' => $banner_image['image_height'],
                    'image_content_data' => $image_content_data
                );
            }
        }
        
        $logger->write("printing banner_images");
        
        $logger->write($data['banner_images']);
    ...
    public function saveImageContent() {
        $logger = new Log("saveImageContent_con");
        $logger->write("inside saveImageContent...");
        ...
        $logger->write($array_data);
        
        $banner_image_id = $this->request->get['banner_image_id'];
        
        
        if ((utf8_strlen(trim($array_data['line_1'])) < 3) || (utf8_strlen(trim($array_data['line_1'])) > 500)) {
            ...
            for ($i=1; $i <= 4; $i++){
                $content_data[] = array(
                    'content' => $array_data['line_'.$i],
                    'sort_order' => $array_data['sort_order_'.$i]
                );
            }
            $this->model_design_banner->saveImageContent($content_data, $banner_image_id);
            $json['message'] = 'Successfully Saved The Image Content!!';
            $json['value'] = 1;
        }
